...
public class NumberConverter {
    private static NumberConverter _instanceBigEndian = null;
    private static NumberConverter _instanceLittleEndian = null;

    public static NumberConverter instanceBigEndian() {
        if (null == _instanceBigEndian) {
            _instanceBigEndian = new NumberConverter();
        }
        return _instanceBigEndian;
    }

    public static NumberConverter instanceLittleEndian() {
        if (null == _instanceLittleEndian) {
            _instanceLittleEndian = new NumberConverter();
            _instanceLittleEndian.resetOrder(ByteOrder.LITTLE_ENDIAN);
        }
        return _instanceLittleEndian;
    }

    public static NumberConverter newInstance(ByteOrder bo) {
        NumberConverter rs = new NumberConverter();
        rs.resetOrder(bo);
        return rs;
    }
    ...
    private ByteBuffer buffer2 = ByteBuffer.allocate(2);
    private ByteBuffer buffer4 = ByteBuffer.allocate(4);
    private ByteBuffer buffer8 = ByteBuffer.allocate(8);
    ...
    /**
     * 设置order 会重围内容全部的ByteBuffer
     *
     * @param bo The new byte order, either BIG_ENDIAN or LITTLE_ENDIAN
     */
     ...
     * @return 整数值
     */
    public final short shortFromString(String value) {
        short iValue = 0;
        try {
     ...
     * @return 整数值
     */
    public final int intFromString(String value) {
        int iValue = 0;
        try {
     ...
     * @return 整数值
     */
    public int toInteger(byte[] values) {
        int len = values.length;
        if (len < 1) {
     ...
     *
     * @param value     整数值
     * @param byteCount 输出位数
     * @return 字节数组
     */
    public byte[] toByteArray(int value, int byteCount) {
        switch (byteCount) {
            case 1:
     ...
     * @return 字节数组
     */
    public byte[] toByteArray(long value, int byteCount) {
        if (8 == byteCount) {
     ...
     * @return 浮点数
     */
    public final float floatFromString(String value) {
        float iValue = 0;
        try {
     ...
     * byte 数组转 float
     *
     * @param values 由字节数组表示的整数
     * @return 浮点数
     */
    public float toFloat(byte[] values) {
        if (null == values) {
            return 0;
        }
     ...
     * @return 浮点数
     */
    public final Double doubleFromString(String value) {
        double iValue = 0;
        try {
     ...
     * @return 浮点数
     */
    public double toDouble(byte[] values) {
        if (null == values) {
            return 0;
        }
        int vLen = values.length;
